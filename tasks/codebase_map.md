...
| `.agent/instructions.md` | active | Boot/close sequence, Open Items table (OI-001–OI-023) |
...
| `DECISIONS.md` | active | Decision log — 30 entries (027/028 hackathon session H1: joint-bundle collection, out-of-band `user_intent`; 029/030 post-H1: Everyday track switch, cache-only public demo serving) |
...
| `api/scan.py` | active | Vercel Python serverless function — the live site's only server-side surface. Imports and calls the actual `bundle.py`/`deep_scan.py`/`anthropic_provider.py`, allowlisted case IDs only, cache-only public responses (DECISIONS.md #030) |
| `public/index.html` | active | The live demo page — hero/damage-story, how-it-works diagram, 3 live case panels (client-side fetch from `/api/scan`), the honest Fable-vs-comparator disclosure table, footer. No build step, no external dependencies |
