...
MODEL_IDS = {
    "fable": FABLE_MODEL,
    # Current comparator for the live demo. Keep this explicit instead of
    # accepting a model ID from the query string.
    "comparator": "claude-opus-5",
}
...
def run_case(case_id, model_key):
    ...
    with open(path, encoding="utf-8") as fh:
        result = json.load(fh)
    recorded_model = result.get("run_record", {}).get("model")
    expected_model = MODEL_IDS[model_key]
    if recorded_model != expected_model:
        return {
            "error": "cached result was generated by a different model; regenerate before serving",
            "case_id": case_id,
            "model_key": model_key,
